...
def get_maximum_weighted_match(left_side: List[str], right_side: List[str]) -> List[Tuple[int, int, float]]:
    ...
    for i, sentence1 in enumerate(left_side):
        for j, sentence2 in enumerate(right_side):
            similatiry = similarity(model, sentence1, sentence2)
...
def get_nodes_for_app(sentences: List[str], start_idx: int, x: int, group: int) -> List[Dict]:
    return [
        {
            "id": i + start_idx, 
            "x": x,
